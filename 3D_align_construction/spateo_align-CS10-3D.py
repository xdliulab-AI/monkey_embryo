...
adata = st.read_h5ad('CS10.h5ad')  
import pandas as pd
df = pd.read_csv('11.3D/SPATEO/CS10_SPATEO_align_metadata.csv',index_col=0)
print(df)
...
st.tdr.save_model(model=embryo_pc, filename="D11.3D/SPATEO/CS10_SPATEO_embryo_pc_model.vtk")
...
st.tdr.save_model(model=embryo_mesh, filename="11.3D/SPATEO/CS10_SPATEO_embryo_mesh_model.vtk")
...
st.tdr.save_model(model=embryo_voxel, filename="11.3D/SPATEO/CS10_SPATEO_embryo_voxel_model.vtk")